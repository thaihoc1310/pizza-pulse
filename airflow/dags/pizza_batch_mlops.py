...
def batch_env_vars(extra_env: list[k8s.V1EnvVar] | None = None) -> list[k8s.V1EnvVar]:
    env = [
        ...
        k8s.V1EnvVar(name="BATCH_RUN_TAG", value="{{ dag_run.run_after.strftime('%Y%m%dt%H%M%S') }}"),
...
with DAG(
    ...
    train_lightgbm = python_batch_pod(
        ...
        pod_name="pizza-train-lightgbm-{{ dag_run.run_after.strftime('%Y%m%dt%H%M%S') }}",
    ...
    train_catboost = python_batch_pod(
        ...
        pod_name="pizza-train-catboost-{{ dag_run.run_after.strftime('%Y%m%dt%H%M%S') }}",
        script_path="/opt/spark/jobs/train_candidate_model.py",
    ...
    compare_and_register_model = python_batch_pod(
        ...
        pod_name="pizza-compare-register-{{ dag_run.run_after.strftime('%Y%m%dt%H%M%S') }}",
